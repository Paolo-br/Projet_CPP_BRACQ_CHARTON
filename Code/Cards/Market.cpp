...
Market::~Market() {
    ...
    // Supprimer les cartes visibles / gemmes de feu / zone de sacrifice détenues
    for (auto card : m_visibleCards) {
        delete card;
    }
    m_visibleCards.clear();

    for (auto gem : m_fireGems) {
        delete gem;
    }
    m_fireGems.clear();
...
void Market::initializeBaseSet() {
    ...
    activateEffs_varrick.push_back(new PutChampionFromDiscardOnTopEffect());
