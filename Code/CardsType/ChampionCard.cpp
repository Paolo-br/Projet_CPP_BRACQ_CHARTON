...
std::tuple<int,int> ChampionCard::display(int row, int col) {
    int indice = 1;
    row++;
    std::vector<Ability*> liste_abi=m_abilities;
    std::string choiceEffect1, choiceEffect2;
    int n = Utils::countChar(Utils::EntireName(getName(),getCost()))-2;
    n = n - Utils::countSpaces(getName());
    if(n<16) n=16;
    //Juste pour éviter le unsued parameter
    ...
    // Afficher l'état du champion (mobilisé / démobilisé / assommé)
    if (isStunned()) {
        Utils::printCentered("ASSOMMÉ", n, "", "bold");
        Utils::moveCursor(row+indice,col);
        indice++;
    } else if (!getIsTapped()) {
        Utils::printCentered("MOBILISÉ", n, "", "bold");
        Utils::moveCursor(row+indice,col);
        indice++;
    } else {
        Utils::printCentered("DÉMOBILISÉ", n, "", "bold");
        Utils::moveCursor(row+indice,col);
        indice++;
    }
    Utils::printBarre(n,1);
    std::cout << std::flush;  // Force le vidage du buffer
    return std::make_tuple(row+indice,n);
}